...
export class History
    ...
    save: (cb) ->
        ...
        history = @commits.slice(- @history-limit)
        <~ @db2.set \history, history

        res = 
            message: "Saved at #{Date!}"
            size: "project: #{parseInt(data.length/1024)}KB, 
                layouts: #{parse-int(JSON.stringify @parent.layouts .length / 1024)}KB, 
                history: #{parse-int(JSON.stringify history .length / 1024)}KB"

        console.log "#{res.message}, #{res.size}"
